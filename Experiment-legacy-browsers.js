...
function AgreementRoutineBegin(snapshot) {
  return async function () {
    ...
    AgreementComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function InstructionPANASRoutineBegin(snapshot) {
  return async function () {
    ...
    InstructionPANASComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function TEXTRoutineEachFrame() {
  return async function () {
    ...
    if (TextResp.status === PsychoJS.Status.STARTED) {
      let theseKeys = TextResp.getKeys({keyList: ['space'], waitRelease: false});
